...
<div class="toolbar no-print">
  ...
  <button onclick="changeEditPassword()">كلمة مرور التعديل</button>
...
function clearStoredPassword() {
  try { localStorage.removeItem(EDIT_PASSWORD_KEY); } catch (e) {}
}

// A failed sync used to show only a small grey line in the toolbar, which is
// easy to miss — so an edit could silently stay on one device. Say it loudly,
// once, until the next successful save.
let syncWarningShown = false;
function failLoudly(msg) {
  if (syncWarningShown) return;
  syncWarningShown = true;
  setTimeout(function () { alert(msg); }, 50);
}

// Lets an editor set or clear the shared-edit password deliberately, instead
// of only being asked after a save has already failed.
function changeEditPassword() {
  const pw = prompt('كلمة مرور التعديل المشترك (اتركها فارغة لمسحها):', getStoredPassword());
  if (pw === null) return;
  if (pw) { setStoredPassword(pw); } else { clearStoredPassword(); }
  syncWarningShown = false;
  setSyncStatus(pw ? 'تم حفظ كلمة المرور في هذا المتصفح' : 'تم مسح كلمة المرور');
}
...
async function syncToServer(key, value, attempt) {
  attempt = attempt || 0;
  try {
    ...
    if (res.status === 401) {
      // A wrong password was cached in this browser: drop it, otherwise every
      // later save keeps failing against the same bad value.
      clearStoredPassword();
      if (attempt < 3) {
        const pw = prompt(attempt === 0
          ? 'كلمة مرور التعديل المشترك:'
          : 'كلمة المرور غير صحيحة. حاول مرة أخرى:');
        if (pw) { setStoredPassword(pw); return syncToServer(key, value, attempt + 1); }
      }
      failLoudly('لم يُحفظ للجميع: كلمة مرور التعديل غير صحيحة أو لم تُدخل. التعديل محفوظ في هذا المتصفح فقط، ولن يظهر لأي شخص آخر.');
      setSyncStatus('لم يُحفظ للجميع (كلمة المرور غير صحيحة) — محفوظ في هذا الجهاز فقط', true);
      return;
    }
    ...
    failLoudly('تعذّر الحفظ للجميع. التعديل محفوظ في هذا المتصفح فقط، ولن يظهر لأي شخص آخر.');
    setSyncStatus('تعذّر الحفظ للجميع — محفوظ في هذا الجهاز فقط', true);
  }
}

async function syncReset(attempt) {
  attempt = attempt || 0;
  try {
    ...
    if (res.status === 401) {
      ...
      if (attempt < 3) {
        const pw = prompt(attempt === 0
          ? 'كلمة مرور التعديل المشترك:'
          : 'كلمة المرور غير صحيحة. حاول مرة أخرى:');
        if (pw) { setStoredPassword(pw); return syncReset(attempt + 1); }
      }
      failLoudly('لم تتم إعادة الضبط للجميع: كلمة مرور التعديل غير صحيحة أو لم تُدخل.');
      setSyncStatus('لم تتم إعادة الضبط للجميع (كلمة المرور غير صحيحة)', true);
      return;
    }
    if (!res.ok) throw new Error('bad status ' + res.status);
    syncWarningShown = false;
    ...
    failLoudly('تعذّرت إعادة الضبط للجميع.');
...
async function loadSharedNumbers() {
  try {
    ...
    if (!res.ok) {
      // The shared store is unreachable (e.g. the server env vars are not
      // set). Say so, instead of silently showing this browser's own values
      // as though they were everyone's.
      setSyncStatus('تعذّر جلب القيم المشتركة — ما تراه قد يكون قيم هذا الجهاز فقط', true);
      return;
    }
    const data = await res.json();
    document.querySelectorAll('[data-key]').forEach(el => {
      // Never overwrite the field the user is typing in right now.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadSavedNumbers();
  loadSharedNumbers();
  // An already-open page picked up other people's edits only on a manual
  // reload; poll, and re-check whenever the tab comes back to the front.
  setInterval(loadSharedNumbers, 30000);
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) loadSharedNumbers();
  });
});
